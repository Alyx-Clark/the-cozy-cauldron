...
```
scenes/
  main_menu.tscn               # Main menu: title + Continue/New Game/Settings/Quit
...
scripts/
  ...
  settings_manager.gd         # AUTOLOAD: persistent settings (volume, fullscreen)
  music_manager.gd            # AUTOLOAD: background music with crossfade
  ...
  ui/
    ...
    main_menu.gd              # Main menu: title, Continue/New Game/Settings/Quit, fade transitions
    settings_panel.gd         # Shared settings UI: volume sliders, fullscreen toggle
    pause_menu.gd             # Pause overlay: Resume/Settings/Main Menu/Quit, ESC coordination
    endgame_popup.gd          # Congratulations popup when all content unlocked
...
assets/music/
  menu_theme.ogg              # CC0 "The Old Tower Inn" by RandomMind (medieval tavern)
  gameplay_theme.ogg          # CC0 "King's Feast" by RandomMind (upbeat medieval folk)
...
|-- PauseMenu (CanvasLayer, pause_menu.gd)     -- [runtime] ESC pause overlay (layer 10)
+-- UI (CanvasLayer)
...
### Main Menu Scene Tree (main_menu.tscn)

```
MainMenu (Control, main_menu.gd)
|-- ColorRect (background, mouse_filter=IGNORE)
|-- VBoxContainer (centered)
|   |-- Title Label ("The Cozy Cauldron", 36px, drop shadow)
|   |-- Subtitle Label ("A Cozy Potion Automation Game")
|   |-- Continue Button (disabled if no save)
|   |-- New Game Button (confirm dialog if save exists)
|   |-- Settings Button (opens SettingsPanel)
|   +-- Quit Button
|-- SettingsPanel (PanelContainer)            -- [transient] volume sliders + fullscreen
+-- ColorRect (fade overlay for scene transitions)
```
...
| `SettingsManager` | `settings_manager.gd` | Persistent settings: volume, fullscreen |
| `MusicManager` | `music_manager.gd` | Background music with crossfade, `MusicManager.play_track("name")` |
...
10. **PauseMenu** - Create CanvasLayer, pass save_manager reference
11. **Load or fresh start** - save_manager.load_game() then _restore_machines()/_restore_player_pos(), or tutorial_manager.show_initial_hint()
12. **Endgame detection** - Connect recipe_unlocked, machine_unlocked, region_unlocked signals
13. **Gameplay music** - MusicManager.play_track("gameplay_theme") (crossfades from menu theme)
...
| ESC | Close shop (if open) / Toggle pause menu |
| Ctrl+S | Manual save |
...
### Phase 7 (Menu + Music + Settings) -- COMPLETE
- **Main menu scene** -- `scenes/main_menu.tscn` + `scripts/ui/main_menu.gd`. Title "The Cozy Cauldron" (36px pixel font, drop shadow) + subtitle. Buttons: Continue (disabled if no save), New Game (confirm dialog if save exists + GameState.reset()), Settings, Quit. Fade-to-black scene transition (0.5s tween). Set as `project.godot` main_scene
- **SettingsManager autoload** -- `scripts/settings_manager.gd`. Persistent at `user://settings.json`: master_volume (1.0), music_volume (0.8), sfx_volume (0.8), fullscreen (false). `signal settings_changed` on any change. Master volume applies to AudioServer bus 0 via `linear_to_db()`. Fullscreen via `DisplayServer.window_set_mode()`
- **MusicManager autoload** -- `scripts/music_manager.gd`. Two AudioStreamPlayers for crossfade (1.0s). Tracks: `menu_theme` + `gameplay_theme` (.ogg). `play_track(name)` crossfades, `stop()` fades out 0.5s, `set_volume_multiplier(mult)` for pause ducking (0.3s tween). Reads music_volume from SettingsManager, connects to `settings_changed`
- **CC0 music** -- "The Old Tower Inn" (menu) + "King's Feast" (gameplay) by RandomMind from opengameart.org. CC0 licensed. Converted from WAV to .ogg Vorbis. Loop enabled in .import files
- **Settings panel** -- `scripts/ui/settings_panel.gd` (class_name SettingsPanel). Shared by main menu and pause menu. Wood panel, 3 HSlider rows (Master/Music/SFX 0-100%) with percentage labels, fullscreen CheckButton, Back button. Emits `closed` signal
- **Pause menu** -- `scripts/ui/pause_menu.gd`. CanvasLayer (layer 10), `process_mode = PROCESS_MODE_ALWAYS`. Dimmer + wood panel + buttons (Resume, Settings, Main Menu, Quit). ESC coordination: `_input()` fires before `_unhandled_input()`, checks if UnlockShop visible before opening. Pause: `get_tree().paused = true`, music ducks to 50%. Main Menu: saves game, changes scene. Quit: saves game
- **SoundManager SFX volume** -- Connects to SettingsManager.settings_changed. `_on_settings_changed()` applies `linear_to_db(sfx_volume) + BASE_VOLUME_DB` to all players
- **Endgame popup** -- `scripts/ui/endgame_popup.gd` (class_name EndgamePopup). Static `show_popup(parent)` API. CanvasLayer (layer 11), wood panel, "Congratulations!" title, stats (all recipes/machines/regions), gold display, "Continue Playing" button. Scale+fade entrance animation. Guarded by `GameState.endgame_shown` flag (persisted in save)
- **GameState.reset()** -- New method resets gold, unlocks, endgame_shown to fresh-start state. Called by main menu on New Game
- **Music wiring** -- Main menu: `play_track("menu_theme")` in `_ready()`. Gameplay: `play_track("gameplay_theme")` at end of main.gd `_ready()` (crossfades naturally from menu theme)
- **Files: New** -- `scenes/main_menu.tscn`, `scripts/ui/main_menu.gd`, `scripts/settings_manager.gd`, `scripts/music_manager.gd`, `scripts/ui/settings_panel.gd`, `scripts/ui/pause_menu.gd`, `scripts/ui/endgame_popup.gd`, `assets/music/menu_theme.ogg`, `assets/music/gameplay_theme.ogg`
- **Files: Modified** -- `project.godot` (main_scene -> main_menu.tscn, 2 new autoloads), `scripts/sound_manager.gd` (SFX volume from SettingsManager), `scripts/game_state.gd` (endgame_shown + reset()), `scripts/save_manager.gd` (save/load endgame_shown), `scripts/main.gd` (PauseMenu creation, endgame detection, gameplay music)

### Phase 8 (Steam Integration) -- PLANNED

GodotSteam addon for basic integration: init, achievements, cloud save. Steam achievements mapped to milestones. Export presets for Windows/Mac/Linux. App icon from pixel art assets. Window title "The Cozy Cauldron"
